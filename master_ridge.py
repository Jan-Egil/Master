...
def model_runner_ridge(num_feats, minbins, dataset, alphas):
    ...
    fitting_time = np.zeros_like(alphas)
    fitting_time_std = np.zeros_like(alphas)

    classification_time = np.zeros_like(alphas)
    classification_time_std = np.zeros_like(alphas)

    total_time = np.zeros_like(alphas)
    ...
    for alphaidx, alpha in enumerate(tqdm(alphas)):
        ...
        falsepos[alphaidx] = np.mean(fpr_list)
        falsepos_std[alphaidx] = np.std(fpr_list)

        fitting_time[alphaidx] = np.mean(fitting_time_list)
        fitting_time_std[alphaidx] = np.std(fitting_time_list)

        classification_time[alphaidx] = np.mean(predicting_time_list)
    ...
    for num_feats in tqdm(num_feats_list):
        for minbins in tqdm(minbins_list):
            for dataset in tqdm(dataset_list):
                model_runner_ridge(num_feats, minbins, dataset, alphas)