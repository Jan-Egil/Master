...
def threshold_scanner(alpha, thresholds):
...
np.save('alphas_array.npy', alphas)
"""
...
alphas = np.load('alphas_array.npy')

print(balaccs_array.shape)

plt.figure()
plt.contourf(thresholds, alphas, balaccs_array)
plt.colorbar(cmap='Greys').set_label('Balanced Accuracy', fontsize='large')
plt.yscale('log')
plt.xlabel("Threshold", fontsize='large')
plt.ylabel("Regularization Hyperparameter $\\alpha$", fontsize='large')
plt.title("Balanced Accuracy\nHyperparameter Plot", fontsize='large')

plt.figure()
plt.contourf(thresholds, alphas, recalls_array)
...
plt.figure()
plt.contourf(thresholds, alphas, fprs_array)
