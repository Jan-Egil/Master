...
for dataset in dataset_list:
    for minbins in minbins_list:
        for num_feats in num_feats_list:
            filepath = f"Ridge_{dataset}_{minbins}bins_{num_feats}.csv"
            ...
            best_alpha = alphas[best_balacc_idx]

            print(f"{dataset} {minbins}bins {num_feats} feats. Alpha = {best_alpha:e}")
            ...
            if num_feats == 35:
                plt.plot(alphas,balaccs, label=f"{dataset} | {minbins}minbin")
plt.xscale('log')
plt.ylabel("Balanced accuracy score", fontsize='large')
plt.xlabel("Hyperparameter $\\alpha$", fontsize='large')
plt.title("Balanced Accuracy against hyperparameter $\\alpha$\nReduced to 35 features")
plt.legend()
plt.show()